...
That meant a launcher app could repeatedly create useful sessions while still lacking a stable place to accumulate code,
scripts, prompts, notes, and artifacts across launches.

This change adds an optional **app-scoped persistent workspace** for launcher apps.

## Why This Was Needed

The immediate driver was a launcher app that needed more than one-shot session startup behavior.
...
- the launcher had no standard way to tell an agent where its durable app-local code should live
...
```text
.aize-state/apps/<username>/<app_id>/workspace/
```

It also records metadata in:

```text
.aize-state/apps/<username>/<app_id>/app.json
```
...
and the launch plan prepends a prompt note telling the agent to use that directory for durable app-local assets.

## Design Intent

This is intentionally minimal.
It does not yet add a full app-local file browser or app-level ACL model.
...
- launcher apps can now keep durable code and artifacts without depending on one specific session directory
