...
- Removed generated `__pycache__` directories under `src/new_aize` and `tests`.
...
- Legacy state migration helpers still exist because live `.new-aize-state` may contain records from earlier same-day schema iterations.
