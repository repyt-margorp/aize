...
2. `unit` layer
   Units define service, session, manager, agent, interface, and device behavior. Units communicate through endpoints and messages.

In practice:

- `service.json` defines executable service kinds.
- `unit.json` defines kernel-managed units for humans, schedulers, and interfaces.
...
- The current minimum unit descriptor contract is documented in `./src/runtime/schemas/unit_file_v1.json`; `plugin_session_template_v1.json` remains as a compatibility filename.
...
- Unit discovery reads `./plugins/**/units/*/unit.json`.
...
- Use units as the stable user-facing entrypoint for starting domain-specific sessions.
