...
- `new_aize.store.Store` remains the public runtime state API.
...
- Split dispatch internals further:
  ...
  - `store_dispatch_queue.py`: dispatch queue selection, retry timing, queue entry lifecycle.
...
PYTHONPATH=src AIZE_ENABLE_EXTERNAL_AGENTS=false python3 -m new_aize.cli --root "$tmp/state" user-input smoke root "hello"
...
- `store.py` still contains same-day migration code. That can be removed later once `.new-aize-state` compatibility with earlier schema iterations is no longer needed.
