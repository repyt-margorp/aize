...
- `python3 -m unittest tests.test_http_handler_goal_save tests.test_entrance_page tests.test_session_template -q`
- Confirmed the Entrance renderer still contains neutral status text, omits the old `InteractiveAgent` statuses, and does not contain the optimistic outbound `renderChat([{direction:'out'...` path.
- Re-ran headless Chrome DevTools Protocol verification against `https://127.0.0.1:64123/units/entrance?session_id=06cfdf8c6dbd301e`; after submitting through the Entrance form, the visible status was `Input sent. Waiting for Entrance updates...`, the form stayed enabled, and the rendered body had no old parallel-agent claim.
