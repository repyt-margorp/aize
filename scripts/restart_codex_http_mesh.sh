...
nohup setsid /bin/bash -lc \
  "cd '$ROOT' && ROOT='$ROOT' AIZE_RUNTIME_ROOT='${AIZE_RUNTIME_ROOT:-$ROOT/.aize-runtime}' AIZE_HTTP_HOST='${AIZE_HTTP_HOST:-127.0.0.1}' AIZE_HTTP_PORT='${AIZE_HTTP_PORT:-4123}' RESTART_LOCK_PATH='$RESTART_LOCK_PATH' SYNC_RESTART=1 '$ROOT/restart_codex_http_mesh.sh' \"\$@\" >>'$DISPATCH_LOG_PATH' 2>&1" \
