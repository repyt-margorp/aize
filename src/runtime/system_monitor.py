...
def _raw_session_history(runtime_root: Path, *, username: str, session_id: str) -> list[dict[str, Any]]:
    entries = read_jsonl(session_timeline_path(runtime_root, username=username, session_id=session_id))
    return [item for item in entries if isinstance(item, dict)]


def _raw_pending_inputs(path: Path) -> list[dict[str, Any]]:
    ...
    with state_read_lock(runtime_root):
        for session in sessions:
            ...
            if not username or not session_id:
                continue
            history_entries = _raw_session_history(runtime_root, username=username, session_id=session_id)
            pending_inputs = _raw_pending_inputs(
                session_pending_path(runtime_root, username=username, session_id=session_id)
            )
            goal_manager_inputs = _raw_pending_inputs(
                session_goal_manager_pending_path(runtime_root, username=username, session_id=session_id)
            )
            pending_user_inputs = [
