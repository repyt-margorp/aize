...
class DispatchMixin:
        ...
        if result["goal_status"] == "completed":
            ...
            if worker_requested:
                ...
                if queue_entry:
                    ...
                    queue_entry["released_at"] = completed_at
                self._ensure_retry_dispatch_entry(
                    state,
                    goal,
                    reason=result["goal_reason"],
                    available_after=self._retry_available_after(),
                )
    ...
    def _set_run_current_phase(self, run_id: str, phase: str) -> None:
        with self._state_lock():
            state = self.load()
            run = state["dispatch_runs"].get(run_id)
            if not run or run.get("lease_state") != "acquired":
                return
    ...
    def _ensure_agent_resume_token(self, *, session_id: str, role: str) -> str:
        with self._state_lock():
            state = self.load()
            thread = self._ensure_agent_thread(state, session_id=session_id, role=role)
            self.save(state)
            return str(thread["resume_token"])
